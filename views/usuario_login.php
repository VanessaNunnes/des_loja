...
<body>
    ...
    <div class="container col-lg-4 col-md-6 col-sm-8 col-10 mt-4 border p-2 g-2">
        ...
        <form action="usuario.php?action=login" method="post" class="row p-2 g-2">
            ...
            <?php if (isset($_SESSION['erro_usuario_nao_logado'])) { ?>
                <div id="msg_erro_usuario" class="alert alert-danger" role="alert">
                    <?= $_SESSION['erro_usuario_nao_logado'] ?>
                </div>
            <?php unset($_SESSION['erro_usuario_nao_logado']);} ?>
...
<script>
    $(document).ready(function(){
        $('#msg_erro_usuario').hide();
        $('#msg_erro_usuario').slideDown(500).delay(5000).slideUp(500);
        $('#msg_erro').hide();
        $('#msg_erro').slideDown(500).delay(5000).slideUp(500);
    });
</script>
